...
CRUD.WorkerBackgroundPageAdapter = function (database, dbOptions) {
...
CRUD.BackgroundPageConnection = function () {
  ...
  this.connect = function () {
    if (connected) {
      console.error('already connected to background page!')
      return
    }

    connected = true
    chrome.runtime.onMessage.addListener(msg => {
      if (msg.__type !== 'CRUDResponse') {
        return
      }

      CRUD.log('received response from background page', msg)

      if (!msg.guid) {
        throw new Error('unidentified port response')
      }
      if (!(msg.guid in unhandled)) {
        throw new Error('port response already handled or unknown' + msg.guid)
      }
      if (!msg.error) {
