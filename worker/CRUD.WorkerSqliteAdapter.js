...
CRUD.WorkerSqliteAdapter = function (database, dbOptions) {
  ...
  var verifyTables = function () {
    ...
    return db.execute('select type,name,tbl_name from sqlite_master').then(function (resultset) {
      return resultset.rows
        .filter(function (row) {
          return !((row.name.indexOf('sqlite_autoindex') > -1 || row.name == '__WebKitDatabaseInfoTable__'))
        })
        .map(function (row) {
          if (row.type == 'table') {
  ...
  var createFixtures = function (entity) {
  ...
  var delayUntilSetupDone = function (func) {
...
CRUD.Database = function (name, options) {
  ...
  function processQueue() {
    if (queryQueue.length > 0) {
      const localQueue = queryQueue.splice(0, 25)
      localQueue.map(function (query) {
        CRUD.log('Executing query', query.sql, query.valueBindings)
        try {
          const rows = db.exec({
            sql: query.sql,
            rowMode: 'object',
            returnValue: 'resultRows',
            bind: query.valueBindings || []
          })

          let lastInsertId
          if (query.sql.trim().toUpperCase().startsWith('INSERT INTO')) {
            lastInsertId = db.exec({
          ...
          console.error('Error executing query', query.sql, query.valueBindings, err)
        }
      })
    }
  }
  ...
  this.connect = async () => {
    try {
      ...
      CRUD.log('DB ERROR ' + err.toString())
      throw err
    }
  }
}
